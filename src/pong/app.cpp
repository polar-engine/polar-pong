...
app::app(polar::core::polar &engine) {
	...
	engine.addstate("root", [](core::polar *engine, core::state &st) {
		...
		st.add_system_as<system::renderer::base, system::renderer::gl32>(
	...
	engine.addstate("game", [](core::polar *engine, core::state &st) {
		...
		engine->add_component<component::position>(leftPaddle,
		                                           Point3(-0.925, 0, 0));
		engine->add_component<component::position>(rightPaddle,
		                                           Point3(0.925, 0, 0));
		engine->add_component<component::position>(ball);

		engine->add_component<component::scale>(leftPaddle,
		                                        Point3(0.025, 0.2, 0));
		engine->add_component<component::scale>(rightPaddle,
		                                        Point3(0.025, 0.2, 0));
		...
		engine->get_component<component::position>(ball)->pos.derivative() =
		    Point3(-0.5, -0.1, 0);
		...
		auto onKey = [engine, speed, leftPaddle](key_t k) {
			auto paddle =
			    engine->get_component<component::position>(leftPaddle);
